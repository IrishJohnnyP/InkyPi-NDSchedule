...
class NdSchedule(BasePlugin):
    ...
    def _fetch_schedule_for_year(self, team_id: int, year: int, ttl: int) -> Dict[str, Any]:
        ...
        candidates = [
            f"{base}?season={year}", f"{base}?year={year}",
            f"{base}?season={year}&seasontype=2", f"{base}?season={year}&seasontype=3",
            f"{base}?year={year}&seasontype=2", f"{base}?year={year}&seasontype=3",
            base,
        ]
        last = None
